...
func Add(v string, f *models.User) (int, string) {
	...
	dao.Create_friend_application(v, f)
	return 200, "申请成功"
}

// 同意好友申请
func Agree(n string, s string) (int, string) {
	dao.Create_friend(n, s)             //数据库插入好友记录
	dao.Create_friend(s, n)             //数据库插入好友记录
	dao.Delete_friend_application(n, s) //删除好友请求
	msg := vo.Message{
		Fr:  s,
		To:  n,
		Ctx: "我已添加你为好友",
...
func Get_messages_list(n string, s string) (int, []map[string]any) {
	v := dao.GetMessages(n, s)
	v = append(v, dao.GetMessages(s, n)...)
...
func CreateGroup(n string, g string) (int, map[string]any) {
	num := dao.SearchGroup(g)
	if num != 0 {
		return 500, gin.H{
			"data": "已有该群",
		}
	}
	e1, e2 := dao.CreateGroup(g, n)
	if e1 != nil || e2 != nil {
		fmt.Println(e1, e2)
		return 500, gin.H{
			"data": "创建失败",
